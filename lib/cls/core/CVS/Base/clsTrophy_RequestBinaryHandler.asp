<!--#INCLUDE FILE="clsTrophy_RequestBinaryField.asp"-->
<%
' ------------------------------------------------------------------------------
'	Author:		Lewis Moten
...
Class Trophy_RequestBinaryHandler
' ------------------------------------------------------------------------------
	...
	Public fieldNames()			' Field names /*ufux*/
	Public fieldDict			' Fields for iterator
	Private mlngCount			' Number of fields parsed
	
	
	Public Function parseRequestData(ByRef bytedata,datasize)
		mbinData = bytedata
		mlngBytesReceived = datasize
...
' ------------------------------------------------------------------------------
	...
	Private Sub loadStack()
		tmpArray = split(request.QueryString,"&")
		For i=0 to UBound(tmpArray)
			pairs = split(tmpArray(i),"=")
			if pairs(0) <> "" then
				Call AddField(pairs(0), "", "", pairs(1), "")
			end if
		Next
	End Sub
' ------------------------------------------------------------------------------
	Private Sub AddField(ByRef pstrName, ByRef pstrFileName, ByRef pstrContentType, ByRef pstrValue, ByRef pbinData)
		...
		ReDim Preserve fieldNames(mlngCount)

		' Create new field object
		Set lobjField = New Trophy_RequestBinaryField
...
' ------------------------------------------------------------------------------
	...
	Public Default Property Get Fields(ByVal pstrName)
		...
		Set Fields = New Trophy_RequestBinaryField
...
' ------------------------------------------------------------------------------
	Private Sub Class_Initialize()
		...
		mlngCount = 0
		Set fieldDict = Server.CreateObject("Scripting.Dictionary")
...
' ------------------------------------------------------------------------------
	...
	Public Function loadIterator()
		For i=0 to UBound(fieldNames)
			Call fieldDict.Add(fieldNames(i),Fields(fieldNames(i)))
		Next
	End Function
End Class
' ------------------------------------------------------------------------------
